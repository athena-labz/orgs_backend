...
async def test_group_task_create():
    ...
    await task.fetch_related("group", "owner_membership")
    ...
    assert task.is_individual == False
    assert task.group.id == created_group.id
    assert task.owner_membership is None
...
async def test_individual_task_create():
    ...
    membership_created = await OrganizationMembership.create(
        user=created_user,
        organization=created_organization,
        area=None,
    )
    ...
    assert task.owner_membership.id == membership_created.id
...
async def test_task_start_approve():
    ...
    student_stake_address = (
        "stake1u9ts4zgjrj4x8y8qp69w3kv4dh8wdpg5s9dnuftw4qjujmg7pnjve"
    )
    ...
    teacher_stake_address = (
        "stake1u8f78w7gpq06u89n4tjsdwywrcadl2nm8mdk6u6wvqwjp2cymxms3"
    )
    ...
    supervisor_stake_address = (
        "stake1uyrqwru6mdn7hp5tfg27929nx80h34ktyaru58pcf5xyvzcf4zjz8"
    )
...
async def test_task_start_reject():
    ...
    student_stake_address = (
        "stake1uxmyeqhe3yfccx0qmd46gsjmte9er0nsell32ef648jtfyq0ljrxq"
    )
    ...
    teacher_stake_address = (
        "stake1u94dxhmwsv62syljj6g8p0pv7kme38tl3kpxtqy9er0z7ug0evza0"
    )
    ...
    supervisor_stake_address = (
        "stake1uxzmzuqz7sx4zejq690jcua8cz6x5ycq9azxnjl9pfw74rcudljz7"
    )
...
async def test_task_submission_create():
    ...
    response = client.post(
        ...
        json={"name": f"{test_name} Submission 1", "description": ""},
        headers={"Authorization": "Bearer " + token},
    )
    assert response.status_code == 400
...
async def test_task_submission_approve():
    ...
    student_stake_address = (
        "stake1uxd5n0anqadkyk3378ycdjyacy244ne5an949f8htqlesksram5lz"
    )
...
async def test_task_submission_reject():
    ...
    student_stake_address = (
        "stake1u9qn37zwqdgv3ymncsh2lldhtd8pavyr8l7m6r6l8qtns8g20uk08"
    )
    ...
    teacher_stake_address = (
        "stake1ux8j2sr3qwc43slymf87ny2vumq920ruar0zp9wclhm27kcw5hk26"
    )
    ...
    supervisor_stake_address = (
        "stake1uxdgvn69twx9wh8mw68q03kyzn7pglk8vmcn4v4jtt4s8wcfp4nqg"
    )
...
async def test_task_submission_review():
    ...
    student_stake_address = (
        "stake1uyf0uuy04xthwx9geeqncltcug7pqvwhn3sjd5g6vqyrkus2xg4fa"
    )
    ...
    teacher_stake_address = (
        "stake1u8j8lra9pwsr9ks966ttknsde9pz3u78s9c5jdmcxdeewlq6csmjz"
    )
    ...
    supervisor_stake_address = (
        "stake1u883nd7zpqmj0c3f9rq7wuxnyxe8z4r3jn72ecl66psgp0c570lk0"
    )
...
async def test_task_fund():
    ...
    other_stake_address = "stake1u9epm4gwcduzjxczgv63tdfsaccvxgaumn4j6af8mnchs0ct3y2wu"
    ...
    created_task = await Task.create(
        ...
        description="",
        deadline=datetime.datetime(2024, 1, 1),
        owner_membership=other_membership,
        is_individual=True,
        is_approved_start=True,
    ...
    await UserBalance.create(
        amount=7_000_000, is_claimed=True, user_member=created_membership
    )

    await UserBalance.create(
        amount=7_000_123, is_escrowed=True, user_member=created_membership
    )

    await UserBalance.create(
        amount=100_000_000, is_claimed=False, user_member=other_membership
    )

    # Should not be able to fund task has insufficient funds
    response = client.post(
    ...
    # Should not be able to fund group task
    created_task.update_from_dict({"is_individual": False})
    await created_task.save()
    ...
    created_task.update_from_dict({"is_individual": True})
    await created_task.save()

    # Should not be able to fund task which has not started yet
    created_task.update_from_dict({"is_approved_start": False})
    await created_task.save()
    ...
    # Should not be able to fund task which has already completed
    created_task.update_from_dict(
        {"is_approved_start": True, "is_approved_completed": True}
    )
    await created_task.save()
    ...
    # Make sure TaskFund was created
    task_fund = await TaskFund.filter(
        Q(user_member=created_membership) & Q(task=created_task)
    ).first()
    assert task_fund is not None

    # Make sure funds are escrowed
    escrowed_balances = await UserBalance.filter(
        Q(user_member=created_membership)
        & Q(is_escrowed=True)
        & Q(escrow_task_fund=task_fund)
    ).all()

    print(escrowed_balances)
    ...
    # Should not be able to do it twice
    await UserBalance.create(
        amount=10_000_000,
        is_claimed=False,
        user_member=created_membership,
